...
bool CModel::LoadMaterial(std::string path, bool dontDelete)
{
	...
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		...
		//	mpMaterials[idx]->Specular()[0] = atof(str[1]);
		//	mpMaterials[idx]->Specular()[1] = atof(str[2]);
		//	mpMaterials[idx]->Specular()[2] = atof(str[3]);
		//}
		//else if (strcmp(str[0], "Ke") == 0) {
		//	mpMaterials[idx]->Emissive()[0] = atof(str[1]);
		//	mpMaterials[idx]->Emissive()[1] = atof(str[2]);
		//	mpMaterials[idx]->Emissive()[2] = atof(str[3]);
		//}
		//先頭がdの時、α値を設定する
